...
**Expense Record**:
The structured fields extracted from an Expense Photo and written as one row in the sheet. Fixed columns: recordId, date, vendor, amount, currency, tax, category, notes, status, capturedAt, telegramUser. `recordId` correlates the Sheet row with the Approval Request.
...
Lifecycle of an Expense Record in the sheet. Starts as pending approval; becomes approved after a human confirms in Google Chat.
_Avoid_: state, workflow status (unless referring to n8n execution state)

**Approval Request**:
A Google Chat message asking a human to accept or reject a pending Expense Record. Delivered as a **1:1 DM with the Chat app** (personal Gmail cannot use spaces). Uses n8n Send-and-Wait Approve / Reject resume links.
